...
```scala example h-[400px] start
Dialog(
  _.id    := "dialog-opening", 
  _.label := "Dialog"
)("Lorem ipsum dolor sit amet, consectetur adipiscing elit."),
Button(
  _.open.dialog("dialog-opening") 
)("Open Dialog")
```

Similarly, you can add `_.close.dialog` to a button _inside_ of a dialog to tell it to close.
