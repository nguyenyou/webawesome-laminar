...
```css forId="horizontal-card" show
.horizontal-card {
  img[slot='media'] {
    max-width: 300px;
  }
...
```scala preview id="horizontal-card" h-[300px]
Card(
  ...
  _.slots.actions(
    Button(
      ...
      Icon(_.name := "ellipsis", _.label := "actions")()
    )
  )
)(
  cls := "horizontal-card",
