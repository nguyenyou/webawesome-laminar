import {readFile, writeFile, CONSTANTS, paths, COMPONENT_CUSTOMIZATIONS, SKIP_COMPONENTS} from "./utils.js";
...
function getJSFacadeType(irType: string, unionValues?: string[]): string {
    // Special case: Handle String | js.Array[String] union type (e.g., wa-select value)
    if (irType === "String | js.Array[String]") {
...
function getScalaAttributeType(
    ...
    // Special case: wa-select value attribute
    // HTML attributes are always strings, even though the JS property can be String | js.Array[String]
    // So we keep the attribute type as HtmlAttr[String]
    if (type === "String | js.Array[String]") {
...
async function generateComponent(component: ComponentIR): Promise<string> {
    ...
    // Check if component has checked attribute (for special imports)
    const hasCheckedAttribute = component.attributes.some(
        attr => attr.name === "checked" && attr.fieldName === "defaultChecked"
    );
    ...
    // Add wildcard import for components with checked attribute to get Signal type
    ...
    // Check if this component has a custom trait
    const customization = COMPONENT_CUSTOMIZATIONS[component.tagName];
    if (customization) {
        writer.writeLine(`import ${customization.importPath}`);
    }
    ...
    // Object declaration with optional custom trait
    const extendsClause = customization
        ? `object ${className} extends WebComponent("${component.tagName}"), ${customization.customTrait}`
    ...
    writer
        ...
        .block(() => {
            ...
            // Attributes section
            const regularAttributes = component.attributes;
            if (regularAttributes.length > 0) {
                writer.writeLine("// -- Attributes --");
                writer.blankLine();

                regularAttributes.forEach((attr) => {
                    // Special handling for checked attribute with defaultChecked fieldName
                    if (attr.name === "checked" && attr.fieldName === "defaultChecked") {
                        ...
                        // Generate lazy object checked with special implementation
                        writer.write(`lazy object checked`).block(() => {
                            writer.write(`def :=(value: Boolean) =`).block(() => {
                                writer.writeLine("Seq(");
                                writer.writeLine("  L.defaultChecked := value,");
                                writer.writeLine("  L.checked        := value");
                                writer.writeLine(")");
                            });
                            writer.write(`def <--(value: Source[Boolean]) =`).block(() => {
                                writer.writeLine("Seq(");
                                writer.writeLine("  L.defaultChecked <-- value,");
                                writer.writeLine("  L.checked <-- value");
                                writer.writeLine(")");
                        ...
                        writer.blankLine();
                        return; // Skip standard attribute generation
                    }
                    ...
                    // Use fieldName if available, otherwise use the attribute name
...
export async function generateAllComponents(): Promise<{
    ...
    for (const component of components) {
        try {
            ...
            if (SKIP_COMPONENTS.has(component.tagName)) {
                continue;
            }
            const className = getComponentClassName(component.tagName);
